...
class RevAddOnRelPostsUpdate {
	...
	private $remote_url_info	= 'http://updates.themepunch.tools/addons/revslider-rel-posts-addon/revslider-rel-posts-addon.php';
	...
	protected function _check_updates() {
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				...
				//update_option('rev_rel_posts-stable-version', $data->full->stable);
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('rev_rel_posts-valid', 'false');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('rev-addon-rel-posts-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rev-addon-rel-posts-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('rev-addon-rel-posts-update-check-short', time());
			...
			$response = wp_remote_post($this->remote_url, array(
				'user-agent' => 'WordPress/'.$wp_version.'; '.get_bloginfo('url'),
				'body' => array(
					'item' => urlencode('revslider-rel-posts-addon'),
					'version' => urlencode(REV_ADDON_REL_POSTS_VERSION),
					'code' => urlencode($purchase)
				)
			));
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('rev_rel_posts-connection', false);
				return false;
			}else{
				update_option('rev_rel_posts-connection', true);
			}
		}
		
		if($force_check == true){ //force that the update will be directly searched
			update_option('rev-addon-rel-posts-update-check', '');
		}
