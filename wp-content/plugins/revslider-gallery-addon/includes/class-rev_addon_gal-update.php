...
class RevAddOnGalUpdate {
	...
	private $remote_url_info	= 'http://updates.themepunch.tools/addons/revslider-gallery-addon/revslider-gallery-addon.php';
	...
	protected function _check_updates() {
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				...
				//update_option('rev_addon_gal-stable-version', $data->full->stable);
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('rev_addon_gal-valid', 'false');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('rev-addon-gal-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rev-addon-gal-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('rev-addon-gal-update-check-short', time());
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('rev_addon_gal-connection', false);
				return false;
			}else{
				update_option('rev_addon_gal-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('rev_addon_gal-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('rev_addon_gal-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('rev_addon_gal-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('rev_addon_gal-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('rev_addon_gal-valid', 'false');
					update_option('rev_addon_gal-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('rev-addon-gal-update-check', '');
		}
